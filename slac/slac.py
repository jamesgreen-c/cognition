...
class SLAC:
    ...
    def train_step(
        ...
        grads, aux = self.loss.grads(key, params, posterior, env_state)
    ...
    def fit(self, rpm_params, env_state, initial_data):
        ...
        posterior = self.loss.get_initial_distribution({"rpm": rpm_params}, initial_data)   # or something along these lines (will break currently)
        mean = posterior.params["mean"]

        # initialise actor and critic networks
        key, init_key = jr.split(jr.PRNGKey(self.config.seed), 2)
        self.params = self.loss.init(init_key, mean, rpm_params)
